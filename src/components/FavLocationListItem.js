...
const FavLocationListItem = ({ onClick, navigation, locationData }) => {
  ...
  useEffect(() => {
    if (long != null) getPrevision(lat, long);
  }, [long]);

  useEffect(() => {
    if (previsionData != null) parseData();
  }, [previsionData]);
  ...
  const parseData = () => {
    var hourly_ = previsionData["hourly"].map((obj) => ({
    ...
    setHourly(hourly_);
    const correctName =
      previsionData["current"]["weather"][0]["description"]
        .charAt(0)
        .toUpperCase() +
      previsionData["current"]["weather"][0]["description"].substring(1);
